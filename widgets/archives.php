...
class GantryWidgetArchives extends GantryWidget {
    ...
    function render($args, $instance){
		...
		if($instance['dropdown']) { ?>
			<select name="archive-dropdown" onchange='document.location.href=this.options[this.selectedIndex].value;'> <option value=""><?php echo esc_attr(__('Select Date')); ?></option> <?php wp_get_archives(apply_filters('widget_archives_dropdown_args', array('type' => $instance['type'], 'format' => 'option', 'show_post_count' => $instance['show_count'], 'limit' => $limit))); ?> </select>
		...
		<?php } else { ?>
			...
			<ul<?php echo $menu_class; ?>>
				...
				<?php 
					...
					$output = wp_get_archives(apply_filters('widget_archives_args', array('type' => $instance['type'], 'show_post_count' => 0, 'limit' => $limit, 'echo' => '0')));
