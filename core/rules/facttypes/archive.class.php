...
class GantryFactArchive extends GantryOverrideFact {
    ...
    public function matchesArchivePage($query) {
        ...
        else if ($query->is_tag && $this->type == "post_tag" && $this->id == $query->query_vars['tag_id']) return true;
        else if ($query->is_category && $this->type == "category" && $this->id == $query->query_vars['cat']) return true;
        return false;
    }

    public function matchesArchiveType($query) {
        if (!$query->is_archive) return false;
        if ($query->is_tax && $this->type == $query->query_vars['taxonomy']) return true;
        else if ($query->is_tag && $this->type == "post_tag") return true;
        else if ($query->is_category && $this->type == "category") return true;
        return false;
    }
