...
function gantry_overrides_innertab_wrappers_pre($field) {
	...
	if (!$field->setinoverride) return "";
    else return '<div class="field-label"><span class="inherit-checkbox"><input name="overridden-'.$field->name.'" type="checkbox"' . $checked . '/></span><span class="base-label">';
}
function gantry_overrides_innertab_wrappers_post($field) {
	if (!$field->setinoverride) return "";
    else return '</span></div>';
}
